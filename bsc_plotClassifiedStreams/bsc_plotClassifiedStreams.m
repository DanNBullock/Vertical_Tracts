function bsc_plotClassifiedStreams(wbFG, classification, t1, figView, saveDir,subSelect,colors)
%
...
slices      = {[-1 0 0],[0 -45 0] ,[0 0 -15]};
%slices      = {[-20 0 0],[0 -73 0] ,[0 0 -5]};
end
...
for iview = 1:length(fig.views)
    ...
    %this preserves axis equivelence across images.  I think.
%     blankView=[0 0 0];
%     blankView(curDim)=1750*sideFlag(iview);
%     campos(blankView);
...
%     feSavefig(fhNum,'verbose','yes',paper ...
%         'figName',strcat(saveDir,fig.names{1},num2str(iview)), ...
%         'figDir',saveDir, ...
%         'figType','jpg');
     %delete(lh)
